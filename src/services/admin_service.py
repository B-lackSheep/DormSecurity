...
class AdminService:
    ...
    async def sync_history(self, bot_manager, limit: int = 120):
        messages = []
        async for message in bot_manager.app.get_chat_history(bot_manager.Config.CHAT_ID, limit=limit):
            if message.text:
                messages.append(f"[{message.date}] {message.text}")

        raw_text = "\n".join(messages)
        llm = LLMService()
        parsed_data = llm.parse_logs_with_dates(raw_text)

        service = CleaningService(self.db)
        count = 0
        for entry in parsed_data:
            res = service.save_duty(entry['room'], entry['date'], entry['notes'])
            if res:
                count += 1
